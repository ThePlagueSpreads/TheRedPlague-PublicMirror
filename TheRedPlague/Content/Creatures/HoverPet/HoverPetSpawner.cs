...
public class HoverPetSpawner : MonoBehaviour
{
    ...
    {
        if (!StoryGoalManager.main.IsGoalComplete(Act2Story.ArriveToDomeGoal.key))
            return false;
        if (StoryGoalManager.main.IsGoalComplete(Act2Story.ScanPlagueAltarGoal.key) ||
            StoryGoalManager.main.IsGoalComplete(Act2Story.MazeBaseDiscoveryGoal.key))
            return false;
        return true;
    }
