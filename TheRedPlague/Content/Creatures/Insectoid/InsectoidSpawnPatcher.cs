...
namespace TheRedPlague.Content.Creatures.Insectoid;
...
public static class InsectoidSpawnPatcher
{
    ...
    private static IEnumerator SpawnInsectoid(Vector3 position, Quaternion rotation, bool jump, string goalKey)
    {
        ...
        var task = PrefabDatabase.GetPrefabAsync("SmallInsectoid");
        yield return task;
        if (!task.TryGetPrefab(out var insectoidPrefab))
            Plugin.Logger.LogError("Failed to find SmallInsectoid prefab!");
        var insectoid = Object.Instantiate(insectoidPrefab, position, rotation);
