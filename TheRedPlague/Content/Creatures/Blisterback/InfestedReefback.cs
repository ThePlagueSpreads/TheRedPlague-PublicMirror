...
using TheRedPlague.Content.Environment.Blister;
using TheRedPlague.Content.Infection;
using TheRedPlague.Framework.SFX;
using TheRedPlague.Utilities;
using UnityEngine;

namespace TheRedPlague.Content.Creatures.Blisterback;
...
public class InfestedReefback : CreatureAsset
{
    ...
    protected override CreatureTemplate CreateTemplate()
    {
        var template = new CreatureTemplate(
            () => AssetBundles.Creatures.LoadAsset<GameObject>("InfestedReefbackPrefab"),
        ...
        CreatureTemplateUtils.SetCreatureDataEssentials(template, LargeWorldEntity.CellLevel.VeryFar, 10400, 0,
            new BehaviourLODData(50, 150, 500), 1000);
        return template;
    }
    ...
    protected override IEnumerator ModifyPrefab(GameObject prefab, CreatureComponents components)
    {
        ...
        var blistersParent = prefab.transform.Find("Blisters");
        foreach (Transform blister in blistersParent)
        {
            ExplodingBlister.SetUpBlister(prefab, blister.gameObject, 0.2f, 9f, 6f, 8f, 25f, 2f, 18f, true);
        }
    ...
    {
        MaterialUtils.ApplySNShaders(prefab, 5f, 1f, 0.05f,
            new FresnelModifier(0f));
    }
}