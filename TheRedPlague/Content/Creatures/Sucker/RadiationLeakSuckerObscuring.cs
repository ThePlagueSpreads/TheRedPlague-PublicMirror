...
public class RadiationLeakSuckerObscuring : MonoBehaviour, IManagedUpdateBehaviour
{
    ...
    public static bool IsSuckerInDriveCoreRoom(Vector3 position)
    {
        return Vector3.SqrMagnitude(position - RoomCenter) < MaxDistanceFromRoom * MaxDistanceFromRoom;
    }
    
    public static void RegisterSucker(Transform transform)
    {
        if (!IsSuckerInDriveCoreRoom(transform.position))
        {
            return;
        }
