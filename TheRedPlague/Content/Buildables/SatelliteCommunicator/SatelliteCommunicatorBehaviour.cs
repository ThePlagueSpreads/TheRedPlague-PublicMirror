...
public class SatelliteCommunicatorBehaviour : MonoBehaviour, IStoryGoalListener, IManagedUpdateBehaviour
{
    ...
    private IEnumerator Start()
    {
        ...
        if (StoryGoalManager.main.IsGoalComplete(Act2Story.TeleportToSpaceEvent.key))
        {
            SetStateDisabled();
            yield break;
        }
        ...
        Act2Story.SatelliteCommunicatorSuccess.Trigger();
    ...
    {
        if (string.Equals(key, Act2Story.TeleportToSpaceEvent.key))
        {
            SetStateDisabled();
        }
    }
