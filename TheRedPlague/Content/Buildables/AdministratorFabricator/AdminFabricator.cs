...
namespace TheRedPlague.Content.Buildables.AdministratorFabricator;
...
public static class AdminFabricator
{
    public static PrefabInfo Info { get; } = PrefabInfo.WithTechType("AdminFabricator")
        .WithIcon(AssetBundles.Core.LoadAsset<Sprite>("AdminFabricatorIcon"));
    ...
    private static void Register()
    {
        ...
        KnownTechHandler.SetAnalysisTechEntry(new KnownTech.AnalysisTech
        {
            ...
            unlockPopup = AssetBundles.Core.LoadAsset<Sprite>("AdminFabricatorPopup"),
    ...
    public static void ApplyAdminFabricatorMaterials(GameObject fabricatorModel)
    {
        foreach (var renderer in fabricatorModel.GetComponentsInChildren<Renderer>(true))
        {
            ...
            foreach (var material in materials)
            {
                ...
                {
                    material.mainTexture = AssetBundles.Core.LoadAsset<Texture2D>("admin_fabricator_diffuse");
                    material.SetTexture("_SpecTex", AssetBundles.Core.LoadAsset<Texture2D>("admin_fabricator_spec"));
                    material.SetTexture("_Illum", AssetBundles.Core.LoadAsset<Texture2D>("adminfabricator_illum"));
                }
            }
        }
    }
