...
namespace TheRedPlague.Content.Buildables.AssimilationGenerator;

[PrefabClass]
public static class AssimilationGenerator
{
    public static PrefabInfo Info { get; } = PrefabInfo.WithTechType("AssimilationGenerator")
        .WithIcon(AssetBundles.Core.LoadAsset<Sprite>("AssimilationGeneratorIcon"));
    ...
    private static void Register()
    {
        ...
        new DataboxPrefab("AssimilationGeneratorDatabox", Info.TechType).Register();
    }

    private static IEnumerator CreatePrefab(IOut<GameObject> result)
    {
        var prefab = Object.Instantiate(AssetBundles.Core.LoadAsset<GameObject>("AssimilationGenerator"));
