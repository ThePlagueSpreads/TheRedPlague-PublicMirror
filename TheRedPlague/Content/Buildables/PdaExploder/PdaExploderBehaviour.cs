...
using Story;
using TheRedPlague.Framework.API;
...
namespace TheRedPlague.Content.Buildables.PdaExploder;
...
public class PdaExploderBehaviour : CinematicModeTrigger, IStoryGoalListener
{
    ...
    private void Start()
    {
        ...
        _canBeActivated = !StoryGoalManager.main.IsGoalComplete(Act2Story.ExplodePda.key);
    ...
    private void OnCinematicEnd(CinematicModeEventData data)
    {
        ...
        KnownTech.Remove(PdaExploder.Info.TechType);
    }
    ...
    private static IEnumerator StartCinematicEvents(PdaExploderBehaviour exploder)
    {
        ...
        if (StoryGoalManager.main.IsGoalComplete(Act2Story.BennetInitialMeeting.key))
            Act2Story.BennetExplodePdaEvent.Trigger();
    ...
    private static IEnumerator GiveNewPda(GameObject newPdaPrefab)
    {
        ...
        var cinematic = Instantiate(AssetBundles.Core.LoadAsset<GameObject>("TadpolePdaRetrievalPrefab"));
    ...
    {
        if (string.Equals(key, Act2Story.ExplodePda.key, StringComparison.OrdinalIgnoreCase))
        {
