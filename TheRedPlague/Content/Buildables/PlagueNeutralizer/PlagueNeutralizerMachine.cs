...
public class PlagueNeutralizerMachine : MonoBehaviour
{
    ...
    public bool CanRemovePlagueCatalyst()
    {
    ...
    public bool RemovePlagueCatalyst()
    {
        for (int i = 0; i < _data.catalystPowers.Length; i++)
        {
            if (Mathf.Approximately(_data.catalystPowers[i], 1f))
            {
                CraftData.AddToInventory(PlagueCatalyst.Info.TechType, 1, false, false);
                _data.catalystPowers[i] = 0f;
                catalysts.UpdateDisplay(_data.catalystPowers);
                return true;
            }
        }
    ...
    private void StartCraftingIngot()
    {
        ...
        if (!_craftAnimationActive)
            StartCraftingAnimation();
        Act2Story.PlagueNeutralizerFirstUse.Trigger();
    }
