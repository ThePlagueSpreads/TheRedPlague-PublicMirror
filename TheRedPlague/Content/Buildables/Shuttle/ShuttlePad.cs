...
using TheRedPlague.Framework.API.Building;
...
public static class ShuttlePad
{
    public static PrefabInfo Info { get; } = PrefabInfo.WithTechType("ShuttlePad")
        .WithIcon(AssetBundles.Core.LoadAsset<Sprite>("ShuttlePadIcon"));
    ...
    private static void Register()
    {
        ...
        KnownTechHandler.SetAnalysisTechEntry(Info.TechType, Array.Empty<TechType>(),
            ...
            AssetBundles.Core.LoadAsset<Sprite>("ShuttlePadPopup"));
        FloatingBuildableUtils.RegisterBuildableAsFloating(Info.TechType, 0.08f);
    }

    private static IEnumerator CreatePrefab(IOut<GameObject> obj)
    {
        var pad = Object.Instantiate(AssetBundles.Core.LoadAsset<GameObject>("ShuttlePad_Prefab"));
        ...
        console.transform.Find("KeypadUI/Icon").GetComponent<Image>().sprite =
            AssetBundles.Core.LoadAsset<Sprite>("ShuttleCargoIcon");
        ...
        pad.AddComponent<ShuttleFlotationModel>().flotationDeviceParent = pad.transform.Find("Pillows").gameObject;
