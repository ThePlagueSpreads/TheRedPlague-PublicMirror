...
public class ShuttlePadStorageContainer : StorageContainer
{
    ...
    private ResourceHintData[] GetResourceHintTypes()
    {
        return new[]
        {
            new ResourceHintData(AmalgamatedBone.Info.TechType,
                AssetBundles.Core.LoadAsset<Sprite>("AmalgamatedBone")),
            new ResourceHintData(MysteriousRemains.Info.TechType,
                AssetBundles.Core.LoadAsset<Sprite>("MysteriousRemainsIcon")),
            new ResourceHintData(WarperHeart.Info.TechType,
                AssetBundles.Core.LoadAsset<Sprite>("WarperHeartIcon")),
            new ResourceHintData(PlagueCatalyst.Info.TechType,
                AssetBundles.Core.LoadAsset<Sprite>("PlagueCrystalIcon")),
            new ResourceHintData(DormantNeuralMatter.Info.TechType,
                AssetBundles.Core.LoadAsset<Sprite>("DormantPlagueMatterIcon")),
            new ResourceHintData(PlagueIngot.Info.TechType,
                AssetBundles.Core.LoadAsset<Sprite>("PlagueIngotIcon"))
        };
    }
    ...
    private void EnableUI()
    {
        ...
        sendButtonIcon.sprite = AssetBundles.Core.LoadAsset<Sprite>("UploadCargoIcon");
        ...
        if (ShouldDisplayRequiredItems())
        {
            ...
            foreach (var hint in _hintData)
            {
                ...
                string tooltipText = obscured
                    ? Language.main.Get("ShuttleHint_" + hint.ItemType)
                    : Language.main.Get(hint.ItemType);

                var itemTooltip = hintImage.gameObject.AddComponent<SimpleTooltip>();
                itemTooltip.text = tooltipText;
                itemTooltip.translate = false;
    ...
    private bool ShouldDisplayRequiredItems()
    {
        return StoryGoalManager.main.IsGoalComplete(StoryUtils.ChecklistUnlockGoalKey) &&
               !StoryGoalManager.main.IsGoalComplete(Act2Story.PlagueResearchMission.key);
    }
