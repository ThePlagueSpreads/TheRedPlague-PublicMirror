...
public class PlagueRefineryMachine : MonoBehaviour
{
        ...
        _defaultGlobalData = new Dictionary<TechType, Bucket<TechType>>
        {
            ...
            },
            {
                MysteriousRemains.Info.TechType, new Bucket<TechType>(
                    new List<BucketEntry<TechType>>
                    {
                        new(new[] { AmalgamatedBone.Info.TechType, PlagueCatalyst.Info.TechType }, 6),
                        new(DormantNeuralMatter.Info.TechType, 4),
                        new(WarperHeart.Info.TechType, 2),
    ...
    private bool TryGetItemFromInventory(TechType itemType)
    {
        ...
        if (itemsWithTechType is not { Count: > 0 })
            return false;

        var item = itemsWithTechType[0].item;
    ...
    private void PlaceItemOnElevator()
    {
        ...
        else if (_currentLootTechType == AmalgamatedBone.Info.TechType)
        {
