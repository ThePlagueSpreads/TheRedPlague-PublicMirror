...
namespace TheRedPlague.Content.Infection.Patches;
...
public static class InfectedMixinPatcher
{
    ...
    private static readonly Texture2D PrecursorDroidRedIllumTexture =
        AssetBundles.Creatures.LoadAsset<Texture2D>("Precursor_Droid_illum_evil");
    ...
    private static bool EvaluateShouldBeInfectedRandomly(TechType techType)
    {
        ...
        if (Act2Story.IsHivemindReleased())
        {
            return random <= 0.10f;
        }
        ...
        if (StoryGoalManager.main.IsGoalComplete(Act1Story.BiochemicalProtectionSuitUnlockEvent.key))
        {
    ...
    public static void UpdateInfectionShadingPostfix(InfectedMixin __instance)
    {
        ...
        {
            var host = __instance.gameObject.GetComponent<RedPlagueHost>();
            if ((host == null || host.mode == RedPlagueHost.Mode.Normal) && !ZombieManager.IsZombie(__instance.gameObject))
            {
                if (host == null)
                    host = __instance.gameObject.AddComponent<RedPlagueHost>();
                ZombieManager.AddZombieBehaviour(host);
            }
        }
        ...
        for (int i = 0; i < __instance.materials.Count; i++)
        {
            ...
            material.SetTexture(ShaderPropertyID._InfectionAlbedomap, EssentialAssets.ZombieInfectionTexture);
            ...
            switch (techType)
            {
                ...
                case TechType.Jellyray:
                    material.color = new Color(2, 1.2f, 0);
                    material.SetColor("_SpecColor", new Color(1, 0.9f, 0.095f));
                    material.SetColor("_GlowColor", new Color(1.3f, 0.19f, 0));
                    break;
            }
        }
