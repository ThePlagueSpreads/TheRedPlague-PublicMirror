...
public static class ZombieManager
{
    ...
    private static readonly Dictionary<TechType, Vector3> MeleeTriggerPositionOverride = new()
    {
        { TechType.Gasopod, new Vector3(0, 0, 3.36f) },
        { TechType.GhostRayBlue, new Vector3(0, 0, 3.1f) },
        { TechType.GhostRayRed, new Vector3(0, 0, 3.1f) }
    };
    ...
    public static void AddZombieBehaviour(RedPlagueHost host)
    {
        ...
        {
            AddMeleeAttack(creatureComponent, techType);
        }
    ...
    private static void OnConsumeInfectedFish(GameObject obj)
    {
    ...
    private static void AddMeleeAttack(Creature creature, TechType techType)
    {
        ...
        var position = MeleeTriggerPositionOverride.TryGetValue(techType, out var overridePos)
            ? overridePos : Vector3.forward * 0.5f;
    ...
    private static void MakeCreatureAggressiveToEcoTargetType(Creature creature, EcoTargetType type)
    {
        ...
        aggressiveComponent.maxRangeScalar = maxHealth > MediumFishMaxHealth ? 40 : 16;
        aggressiveComponent.ignoreVehicles = maxHealth < MediumFishMaxHealth;
        aggressiveComponent.maxSearchRings = 1;
        aggressiveComponent.aggressionPerSecond = 0.3f;
    ...
    private static void AddAttackLastTarget(Creature creature)
    {
        ...
        attackLastTarget.swimVelocity = swimRandom != null ? swimRandom.swimVelocity * 1.5f : 10f;
    ...
    public static void InfectSeaEmperor(GameObject seaEmperor)
    {
        ...
        foreach (var r in renderers)
        {
            ...
            foreach (var m in materials)
            {
                ...
                m.SetTexture(ShaderPropertyID._InfectionAlbedomap, EssentialAssets.ZombieInfectionTexture);
            }
        }
    }
