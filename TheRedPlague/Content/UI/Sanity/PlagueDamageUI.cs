...
namespace TheRedPlague.Content.UI.Sanity;

public class PlagueDamageUI : MonoBehaviour, IScheduledUpdateBehaviour
{
    ...
    public static PlagueDamageUI Create()
    {
        ...
        droopImage.raycastTarget = false;
        ...
        var chargeIcon = AssetBundles.Core.LoadAsset<Sprite>("BiochemicalChargeIcon");
        ...
        suitChargeBar.insaneSprite = AssetBundles.Core.LoadAsset<Sprite>("InsanityEyeIcon");
        ...
        string outlineName)
    {
        ...
        background.GetComponent<Image>().sprite = AssetBundles.Core.LoadAsset<Sprite>(backgroundName);
        background.transform.GetChild(0).GetComponent<Image>().sprite =
            AssetBundles.Core.LoadAsset<Sprite>(outlineName);
    ...
    private void LoadSprites()
    {
        ...
        {
            _droopProgressSprites[i] = AssetBundles.Core.LoadAsset<Sprite>($"PD_{i + 1:00}");
        }
    }
    ...
    private void Update()
    {
        SetActiveProgress(GetActualProgress());
    ...
    private float GetActualProgress()
    {
        ...
        return InsanityManager.Main.Insanity / 100f;
    }
    ...
    {
        return "TRP:PlagueDamageUI";
    }

    public void ScheduledUpdate()
    {
        var playerBiome = WaterBiomeManager.main.GetBiome(Player.main.transform.position);
        _inPlagueZone = ZombieManager.IsBiomeHeavilyInfected(playerBiome);
    }
}