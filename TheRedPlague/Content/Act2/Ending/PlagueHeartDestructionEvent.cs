...
using TheRedPlague.Framework.API.PDA;
...
public class PlagueHeartDestructionEvent : MonoBehaviour
{
    ...
    public static bool EventIsActive => _instance != null;
    ...
    private IEnumerator Start()
    {
        ...
        bool obeyed = StoryGoalManager.main.IsGoalComplete(Act2Story.ObeyBennetEvent.key);

        // START THE EVENT!

        Act2Story.Act2EndingEvent.Trigger();

        TrpEventMusicPlayer.PlayMusic(Music, 207, true, true);

        Utils.PlayFMODAsset(Music, Player.main.transform.position);
        
        if (BepInEx.Bootstrap.Chainloader.PluginInfos.ContainsKey("com.lee23.epicweather"))
        {
            ModMessageSystem.Send("com.lee23.epicweather", "SetWeather", "ClearSkies");
            ModMessageSystem.Send("com.lee23.epicweather", "SetWeatherPaused", true, true);
        }
        ...
        yield return Teleport(TeleportPositionAboveDome, 2,
            () =>
            {
                ...
                StartSpaceAnimation();
                Act2Story.TeleportToSpaceEvent.Trigger();
            });
        ...
        var strikeParticle = Instantiate(AssetBundles.Core.LoadAsset<GameObject>("SatelliteStrikeParticle"));
        ...
        Act2Story.HiveMindReleasedGoal.Trigger();
    ...
    private void OnDestroy()
    {
        ...
        if (BepInEx.Bootstrap.Chainloader.PluginInfos.ContainsKey("com.lee23.epicweather"))
        {
            ModMessageSystem.Send("com.lee23.epicweather", "SetWeatherPaused", false, false);
        }
    }
    ...
    private IEnumerator FetchReferences()
    {
        ...
        _chaosPrefab = AssetBundles.Creatures.LoadAsset<GameObject>("ChaosLeviathanEndCinematicPrefab");
        ...
        _shuttlePrefab = AssetBundles.Core.LoadAsset<GameObject>("ShuttlePrefab");
        _bennetPrefab = AssetBundles.Characters.LoadAsset<GameObject>("BennetFleshFormBirthPrefab");
    }
    ...
    {
        var cinematic = Instantiate(AssetBundles.Core.LoadAsset<GameObject>(
    ...
    private IEnumerator Teleport(Vector3 position, float paddingDuration, Action actionDuring = null)
    {
        ...
        // Do this TWICE in case the first one fails. This is not a mistake of duplicated code, I swear!
        player.ExitLockedMode();
    ...
    {
        _spaceAnimation = Instantiate(AssetBundles.Core.LoadAsset<GameObject>("SpaceCinematicPrefab"),
    ...
    {
        var prefab = AssetBundles.Core.LoadAsset<GameObject>("PlagueHeartSatelliteExplosion");
