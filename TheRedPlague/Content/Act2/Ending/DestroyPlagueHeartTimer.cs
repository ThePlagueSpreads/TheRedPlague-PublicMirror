...
public class DestroyPlagueHeartTimer : MonoBehaviour, IManagedUpdateBehaviour, IStoryGoalListener
{
    ...
    {
        Main = this;
        if (StoryGoalManager.main.IsGoalComplete(Act2Story.Act2EndingEvent.key))
        {
            Destroy(gameObject);
            return;
        }
    ...
    {
        BehaviourUpdateUtils.Deregister(this);
        if (!StoryGoalManager.main.IsGoalComplete(Act2Story.ObeyBennetEvent.key))
        {
    ...
    {
        if (string.Equals(key, Act2Story.Act2EndingEvent.key, StringComparison.OrdinalIgnoreCase))
        {
            Destroy(gameObject);
        }
    }
