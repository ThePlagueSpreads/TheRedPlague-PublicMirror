...
public class FakeCreatureHallucination : TimedHallucinationSymptom
{
    ...
    protected override float MinInterval => 22;
    protected override float MaxInterval => 36;
    ...
    protected override float FailDelayMultiplier => 0.7f; // shorter delay after a failure
    ...
    private void DisableBehaviours(GameObject creature)
    {
        if (creature.TryGetComponent<EMPAttack>(out var emp))
        {
            emp.evaluatePriority = 0;
        }

        if (creature.TryGetComponent<RangedAttackLastTarget>(out var rangedAttack))
        {
            rangedAttack.evaluatePriority = 0;
        }
    }
