...
public class PdaEyeHallucination : InsanitySymptom
{
    ...
    {
        var model = AssetBundles.Creatures.LoadAsset<GameObject>("PDACameraEyePrefab");
        if (model == null)
        {
    ...
    protected override bool ShouldDisplaySymptoms()
    {
        ...
        if (_storyProgressionAcquired && StoryGoalManager.main.IsGoalComplete(Act2Story.ExplodePda.key))
        {
            return false;
        }
    ...
    {
        return StoryGoalManager.main.IsGoalComplete(Act2Story.BennetIntroduction.key);
    }
