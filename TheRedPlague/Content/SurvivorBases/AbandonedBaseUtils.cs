...
namespace TheRedPlague.Content.SurvivorBases;
...
public static class AbandonedBaseUtils
{
    ...
    public static void StripComponents(GameObject gameObject, Color emissionColor, bool darkenSpecular)
    {
        ...
        // Fix brightness at night
        var renderers = gameObject.GetComponentsInChildren<Renderer>();
        foreach (var renderer in renderers)
        {
            foreach (var material in renderer.materials)
            {
                if (material.HasProperty(IblReductionAtNight))
                    material.SetFloat(IblReductionAtNight, 0.99f);

                if (!darkenSpecular) continue;
