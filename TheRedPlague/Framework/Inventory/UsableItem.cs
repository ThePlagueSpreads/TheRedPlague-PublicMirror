...
public class UsableItem : MonoBehaviour
{
    ...
    private static readonly Dictionary<string, Action<GameObject>> CachedActions = new();
    
    private Action<GameObject> _uniqueAction;
    private bool _useUniqueAction;

    // For use at patch-time.
    public void SetOnUseAction(string classId, Action<GameObject> action)
    {
        cachedId = classId;
    ...
    public void SetUniqueUseAction(Action<GameObject> action)
    {
    ...
    public void ExecuteOnUseAction()
    {
        if (_useUniqueAction)
        {
            _uniqueAction.Invoke(gameObject);
            return;
        }
